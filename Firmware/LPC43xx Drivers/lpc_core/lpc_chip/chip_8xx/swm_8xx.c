...
{
	uint32_t temp;
	temp = LPC_SWM->PINASSIGN[PINASSIGN_IDX(movable)] & (~(0xFF << PINSHIFT(movable)));
	LPC_SWM->PINASSIGN[PINASSIGN_IDX(movable)] = temp | (pin << PINSHIFT(movable));
}
...
{
	if (enable) {	/* clear the bit */
		LPC_SWM->PINENABLE0 &= ~(1 << pin);
	}
	else {	/* set the bit */
		LPC_SWM->PINENABLE0 |= (1 << pin);
	}
...
/* returns true if pin function is enabled, false if disabled */
bool Chip_SWM_IsEnabled(CHIP_SWM_PIN_FIXED_T pin)
{
	if (LPC_SWM->PINENABLE0 & (1 << pin)) {
		return false;	/* the bit is set, has not been enabled */
	}
	return true;/* the bit is not set, therefore is enabled */
}
