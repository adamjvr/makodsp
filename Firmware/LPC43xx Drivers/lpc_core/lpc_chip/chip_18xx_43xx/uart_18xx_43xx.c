...
static CHIP_CCU_CLK_T Chip_UART_DetermineClk(LPC_USART_T *pUART) {
	CHIP_CCU_CLK_T uartclk;
...
void Chip_UART_Init(LPC_USART_T *pUART)
{
	...
	Chip_Clock_EnableOpts(Chip_UART_DetermineClk(pUART), true, true, 1);
...
void Chip_UART_DeInit(LPC_USART_T *pUART)
{
	...
	Chip_Clock_Disable(Chip_UART_DetermineClk(pUART));
}
...
Status Chip_UART_SetBaud(LPC_USART_T *pUART, uint32_t baudrate)
{
	...
	/* Get UART clock rate */
	uClk = Chip_Clock_GetRate(Chip_UART_DetermineClk(pUART));
