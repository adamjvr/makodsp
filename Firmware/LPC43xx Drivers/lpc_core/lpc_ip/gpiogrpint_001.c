...
{
	volatile uint32_t wordDat;
        bool pinstate;

	/* configure to PORT_ENA register */
	pGPIOGPINT->PORT_ENA[PortNum] |= (1<<PinNum);

	/* configure to PORT_POL */
	wordDat = pGPIOGPINT->PORT_POL[PortNum];
        pinstate = (bool) (((wordDat>>PinNum)&0x01) != 0);
	if (pinstate != ActiveMode) {
		if (ActiveMode == true) { /* active HIGH */
			wordDat |= (1<<PinNum);
		} else { /* active LOW */
			wordDat &= ~(1<<PinNum);
		}
		pGPIOGPINT->PORT_POL[PortNum] = wordDat;		
	}
}
