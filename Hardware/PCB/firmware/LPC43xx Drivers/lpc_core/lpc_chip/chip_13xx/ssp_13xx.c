...
{
	if (xf_setup->tx_data) {
		IP_SSP_SendFrame(pSSP, (*(uint16_t *) ((uint32_t) xf_setup->tx_data +
											   xf_setup->tx_cnt)));
	}
	else {
		IP_SSP_SendFrame(pSSP, 0xFFFF);
	}
...
{
	if (xf_setup->tx_data) {
		IP_SSP_SendFrame(pSSP, (*(uint8_t *) ((uint32_t) xf_setup->tx_data + xf_setup->tx_cnt)));
	}
	else {
		IP_SSP_SendFrame(pSSP, 0xFF);
	}
...
void Chip_SSP_DeInit(LPC_SSP_T *pSSP)
{
	...
	if (pSSP == LPC_SSP1) {
		...
		Chip_Clock_SetSSP1ClockDiv(0);
	}
	else {
		Chip_Clock_DisablePeriphClock(SYSCTL_CLOCK_SSP0);
		Chip_Clock_SetSSP0ClockDiv(0);
	}
}
